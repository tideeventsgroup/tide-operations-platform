...
export function EventQuickActions({ incident, priorities }: { incident: Incident; priorities: Priority[] }) {
    ...
    <div className="space-y-3 rounded-lg border border-border bg-card p-4">
      <div className="flex flex-wrap gap-2">
        {incident.status === "reported" ? (
          <Button size="sm" variant="outline" disabled={pending} onClick={() => run(() => acknowledgeEventAction(incident.id))}>
            Acknowledge
          </Button>
        ) : null}
      ...
      {active === "resolve" ? (
        <div className="flex items-center gap-2 border-t border-border pt-3">
          <Input
            ...
            className="flex-1"
          />
          <Button size="sm" disabled={pending || !text.trim()} onClick={() => run(() => resolveEventAction(incident.id, text.trim()))}>
            Confirm resolve
          </Button>
        </div>
      ) : null}
      ...
      {active === "close" ? (
        <div className="flex items-center gap-2 border-t border-border pt-3">
          <Input
            ...
            className="flex-1"
          />
          <Button size="sm" disabled={pending || !text.trim()} onClick={() => run(() => closeEventAction(incident.id, text.trim()))}>
            Confirm close
          </Button>
        </div>
      ) : null}
      ...
      {active === "reopen" ? (
        <div className="flex items-center gap-2 border-t border-border pt-3">
          ...
          <Button
            ...
            onClick={() => run(() => reopenEventAction(incident.id, text.trim()))}
          >
            Confirm reopen
          </Button>
        </div>
