...
import {
  cancelEventActionAction,
  completeEventActionAction,
  createEventActionAction,
} from "@/lib/actions/event-coordination";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Badge } from "@/components/ui/badge";
import { cn } from "@/lib/utils";
import type { listEventActions } from "@/lib/domain/event-service";

type Action = Awaited<ReturnType<typeof listEventActions>>[number];
...
export function EventActionsPanel({ eventId, actions }: { eventId: string; actions: Action[] }) {
  ...
  function submit() {
    ...
    startTransition(async () => {
      const result = await createEventActionAction(eventId, description.trim());
  ...
  function complete(actionId: string) {
    startTransition(async () => {
      const result = await completeEventActionAction(eventId, actionId);
      if (result.error) toast.error(result.error);
    });
  }
  ...
  function cancel(actionId: string) {
    ...
    startTransition(async () => {
      const result = await cancelEventActionAction(eventId, actionId, reason.trim());
      if (result.error) toast.error(result.error);
      else {
