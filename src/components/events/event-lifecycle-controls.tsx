...
import { activateEventAction, changeEventStageAction } from "@/lib/actions/events";
...
const PRE_LIVE_STAGES: Enums<"event_lifecycle_stage">[] = [
  "enquiry",
  "proposal",
  "confirmed",
  "planning",
...
const POST_LIVE_STAGES: Enums<"event_lifecycle_stage">[] = [
  "stand_down",
  "post_event_review",
  "closed",
  "archived",
...
export function EventLifecycleControls({
  ...
  eventId: string;
  stage: Enums<"event_lifecycle_stage">;
  eventControlManagerName?: string;
}) {
  const [pending, startTransition] = useTransition();
  const [nextStage, setNextStage] = useState<Enums<"event_lifecycle_stage">>(stage);
  ...
  function submitStageChange() {
    startTransition(async () => {
      const result = await changeEventStageAction(eventId, nextStage);
  ...
  function submitActivate() {
    startTransition(async () => {
      const result = await activateEventAction(eventId, comments || undefined);
      if (result.error) toast.error(result.error);
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 rounded-lg border border-border bg-card px-4 py-3">
      <select
        value={nextStage}
        onChange={(e) => setNextStage(e.target.value as Enums<"event_lifecycle_stage">)}
        ...
        <AlertDialog>
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This starts live operations. Event Control, incident reporting, and all live-ops surfaces
