...
import { recordEventDecisionAction } from "@/lib/actions/event-coordination";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import type { listEventDecisions } from "@/lib/domain/event-service";

type Decision = Awaited<ReturnType<typeof listEventDecisions>>[number];
...
export function EventDecisionsPanel({ eventId, decisions }: { eventId: string; decisions: Decision[] }) {
  ...
  function submit() {
    ...
    startTransition(async () => {
      const result = await recordEventDecisionAction(eventId, decision.trim(), rationale.trim() || undefined);
