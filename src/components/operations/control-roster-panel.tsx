...
export function ControlRosterPanel({ operationId, roles, sessions }: { operationId: string; roles: Role[]; sessions: Session[] }) {
  ...
  function signOn() {
    ...
    startTransition(async () => {
      const result = await startControlSessionAction(operationId, roleId);
  ...
  function signOff(sessionId: string) {
    startTransition(async () => {
      const result = await endControlSessionAction(operationId, sessionId);
      if (result.error) toast.error(result.error);
    });
  }
