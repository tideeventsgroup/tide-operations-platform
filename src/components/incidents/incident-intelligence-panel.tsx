...
} from "@/lib/actions/incident-intelligence";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Badge } from "@/components/ui/badge";
import type { listIncidentPeople, listIncidentVehicles } from "@/lib/domain/incident-service";
import type { Enums } from "@/lib/supabase/types";

type PersonLink = Awaited<ReturnType<typeof listIncidentPeople>>[number];
type VehicleLink = Awaited<ReturnType<typeof listIncidentVehicles>>[number];
...
export function IncidentIntelligencePanel({
  ...
  return (
    <div className="space-y-8">
      ...
      <PeopleSection incidentId={incidentId} organisationId={organisationId} people={people} />
      <VehiclesSection incidentId={incidentId} organisationId={organisationId} vehicles={vehicles} />
    </div>
  );
}

function PeopleSection({ incidentId, organisationId, people }: { incidentId: string; organisationId: string; people: PersonLink[] }) {
  ...
  function linkExisting(personId: string) {
    ...
    startTransition(async () => {
      const result = await linkExistingPersonAction(incidentId, personId, roleCode.trim());
  ...
  function createNew() {
    ...
    startTransition(async () => {
      const result = await createPersonAction(incidentId, purpose.trim(), roleCode.trim(), {
...
function VehiclesSection({ incidentId, organisationId, vehicles }: { incidentId: string; organisationId: string; vehicles: VehicleLink[] }) {
  const [mode, setMode] = useState<"closed" | "search" | "new">("closed");
  const [query, setQuery] = useState("");
  ...
  function linkExisting(vehicleId: string) {
    if (!roleCode.trim()) return;
    startTransition(async () => {
      const result = await linkExistingVehicleAction(incidentId, vehicleId, roleCode.trim());
      if (result.error) toast.error(result.error);
      else reset();
    });
  }
  ...
  function createNew() {
    if (!purpose.trim() || !roleCode.trim()) return;
    startTransition(async () => {
      const result = await createVehicleAction(incidentId, purpose.trim(), roleCode.trim(), {
