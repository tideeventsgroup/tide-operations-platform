...
import { notifyIncidentAgencyAction, updateIncidentAgencyStatusAction } from "@/lib/actions/incident-agencies";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Badge } from "@/components/ui/badge";
import { cn } from "@/lib/utils";
import type { listIncidentAgencies } from "@/lib/domain/incident-service";
import type { Enums } from "@/lib/supabase/types";

type Agency = Awaited<ReturnType<typeof listIncidentAgencies>>[number];
type AgencyStatus = Enums<"incident_agency_status">;
...
export function IncidentAgenciesPanel({ incidentId, agencies }: { incidentId: string; agencies: Agency[] }) {
  ...
  function submit() {
    ...
    startTransition(async () => {
      const result = await notifyIncidentAgencyAction(
        incidentId,
        agencyType.trim(),
        agencyName.trim(),
  ...
  function advance(agencyId: string, status: AgencyStatus) {
    startTransition(async () => {
      const result = await updateIncidentAgencyStatusAction(incidentId, agencyId, status);
      if (result.error) toast.error(result.error);
    });
  }
