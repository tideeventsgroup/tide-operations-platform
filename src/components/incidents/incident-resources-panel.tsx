...
import { requestIncidentResourceAction, updateIncidentResourceStatusAction } from "@/lib/actions/incident-coordination";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Badge } from "@/components/ui/badge";
import { cn } from "@/lib/utils";
import type { listIncidentResources } from "@/lib/domain/incident-service";
import type { Enums } from "@/lib/supabase/types";

type Resource = Awaited<ReturnType<typeof listIncidentResources>>[number];
type ResourceStatus = Enums<"incident_resource_status">;
...
export function IncidentResourcesPanel({ incidentId, resources }: { incidentId: string; resources: Resource[] }) {
  ...
  function submit() {
    ...
    startTransition(async () => {
      const result = await requestIncidentResourceAction(incidentId, resourceType.trim(), description.trim() || undefined);
  ...
  function advance(resourceId: string, status: ResourceStatus) {
    startTransition(async () => {
      const result = await updateIncidentResourceStatusAction(incidentId, resourceId, status);
      if (result.error) toast.error(result.error);
    });
  }
