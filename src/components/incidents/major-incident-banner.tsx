...
import type { getActiveMajorIncidentActivation } from "@/lib/domain/incident-service";
...
export function MajorIncidentBanner({ incidentId, activation }: { incidentId: string; activation: Activation }) {
  ...
  function activate() {
    ...
    startTransition(async () => {
      const result = await activateMajorIncidentAction(incidentId, reason.trim());
  ...
  function deactivate() {
    startTransition(async () => {
      const result = await deactivateMajorIncidentAction(incidentId);
      if (result.error) toast.error(result.error);
    });
  }
