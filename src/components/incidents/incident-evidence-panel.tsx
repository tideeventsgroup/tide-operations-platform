...
export function IncidentEvidencePanel({ incidentId, eventId, items }: { incidentId: string; eventId: string; items: EvidenceItem[] }) {
    ...
    <div className="space-y-4">
      ...
      <div className="divide-y divide-border rounded-lg border border-border bg-card">
        {items.length === 0 ? (
          ...
          items.map((item) => <EvidenceRow key={item.id} incidentId={incidentId} item={item} />)
        )}
      </div>
    </div>
  );
...
function EvidenceRow({ incidentId, item }: { incidentId: string; item: EvidenceItem }) {
  ...
  function advance(status: EvidenceStatus, notes?: string) {
    startTransition(async () => {
      const result = await updateEvidenceStatusAction(incidentId, item.id, status, notes || undefined);
      if (result.error) toast.error(result.error);
      else {
