...
import { getPortalEvent, getPortalEventSummary, listPortalDocuments } from "@/lib/domain/portal-service";
...
export default async function PortalEventDetailPage({ params }: PageProps<"/portal/operations/[id]">) {
  ...
  const [documents, incidentSummary] = await Promise.all([listPortalDocuments(id), getPortalEventSummary(id)]);
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-8 px-8 py-8">
      ...
      <StatTileGroup>
        <StatTile label="Total events" value={incidentSummary?.total_events ?? 0} />
        <StatTile label="Open" value={incidentSummary?.open_events ?? 0} />
        <StatTile label="Resolved" value={incidentSummary?.resolved_events ?? 0} />
        <StatTile label="Closed" value={incidentSummary?.closed_events ?? 0} />
      </StatTileGroup>
