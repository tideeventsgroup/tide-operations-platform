...
import { searchPeople, searchVehicles } from "@/lib/domain/incident-service";
...
export async function linkExistingPersonAction(
  ...
  return callRpc(
    "link_existing_person_to_incident",
    { p_incident_id: incidentId, p_person_id: personId, p_role_code: roleCode, p_notes: notes || undefined },
    `/incidents/${incidentId}`,
  );
}
...
export async function linkExistingVehicleAction(
  ...
  return callRpc(
    "link_existing_vehicle_to_incident",
    { p_incident_id: incidentId, p_vehicle_id: vehicleId, p_role_code: roleCode, p_notes: notes || undefined },
    `/incidents/${incidentId}`,
  );
}
